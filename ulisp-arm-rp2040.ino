...
#define MEMBANK
  ...
  // TFT 2 - 240x320 2.0 inch TFT
  //#include <Adafruit_ST7789.h> // Hardware-specific library for ST7789
  //#define TFT2_CS 7
  //#define TFT2_DC 6
  //#define TFT2_RST 5 // RST can be set to -1 if you tie it to Arduino's reset
  //Adafruit_ST7789 tft2 = Adafruit_ST7789(TFT2_CS, TFT2_DC, TFT2_RST);
  Adafruit_GFX *tftp = &tft1;

  // Adafruit TFT Scroller
...
enum stream { SERIALSTREAM, I2CSTREAM, SPISTREAM, SDSTREAM, WIFISTREAM, STRINGSTREAM, GFXSTREAM, LOGSTREAM };
...
const char logstream[] PROGMEM = "log";
const char *const streamname[] PROGMEM = {serialstream, i2cstream, spistream, sdstream, wifistream, stringstream, gfxstream, logstream};
...
#endif
#if defined(gfxsupport)
inline void gfxwrite (char c) { (*tftp).write(c); }
inline void logwrite (char c) { logger.write(c); }
#endif
...
pfun_t pstreamfun (object *args) {
  ...
  else if (streamtype == LOGSTREAM) pfun = (pfun_t)logwrite;
  #endif
  #if defined(ULISP_WIFI)
...
#elif defined(ARDUINO_PYBADGE_M4)
  if (!(pin==3 || pin==4 || pin==9 || (pin>=11 && pin<=16) || (pin>=24 && pin<=25) || (pin>=46 && pin<=47))) error(invalidpin, number(pin));
#elif defined(ARDUINO_PYGAMER_M4)
...
object *fn_drawpixel (object *args, object *env) {
  ...
  (*tftp).drawPixel(checkinteger(first(args)), checkinteger(second(args)), colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawline (object *args, object *env) {
  ...
  (*tftp).drawLine(params[0], params[1], params[2], params[3], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawrect (object *args, object *env) {
  ...
  for (int i=0; i<4; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  (*tftp).drawRect(params[0], params[1], params[2], params[3], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_fillrect (object *args, object *env) {
  ...
  for (int i=0; i<4; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  (*tftp).fillRect(params[0], params[1], params[2], params[3], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawcircle (object *args, object *env) {
  ...
  if (args != NULL) colour = checkinteger(car(args));
  (*tftp).drawCircle(params[0], params[1], params[2], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_fillcircle (object *args, object *env) {
  ...
  for (int i=0; i<3; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  (*tftp).fillCircle(params[0], params[1], params[2], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawroundrect (object *args, object *env) {
  ...
  if (args != NULL) colour = checkinteger(car(args));
  (*tftp).drawRoundRect(params[0], params[1], params[2], params[3], params[4], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_fillroundrect (object *args, object *env) {
  ...
  for (int i=0; i<5; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  (*tftp).fillRoundRect(params[0], params[1], params[2], params[3], params[4], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawtriangle (object *args, object *env) {
  ...
  if (args != NULL) colour = checkinteger(car(args));
  (*tftp).drawTriangle(params[0], params[1], params[2], params[3], params[4], params[5], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_filltriangle (object *args, object *env) {
  ...
  for (int i=0; i<6; i++) { params[i] = checkinteger(car(args)); args = cdr(args); }
  if (args != NULL) colour = checkinteger(car(args));
  (*tftp).fillTriangle(params[0], params[1], params[2], params[3], params[4], params[5], colour);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_drawchar (object *args, object *env) {
  ...
  (*tftp).drawChar(checkinteger(first(args)), checkinteger(second(args)), checkchar(third(args)),
    colour, bg, size);
  #else
  (void) args;
  #endif
...
object *fn_setcursor (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  (*tftp).setCursor(checkinteger(first(args)), checkinteger(second(args)));
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_settextcolor (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  if (cdr(args) != NULL) (*tftp).setTextColor(checkinteger(first(args)), checkinteger(second(args)));
  else (*tftp).setTextColor(checkinteger(first(args)));
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_settextsize (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  (*tftp).setTextSize(checkinteger(first(args)));
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_settextwrap (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  (*tftp).setTextWrap(first(args) != NULL);
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_setrotation (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  (*tftp).setRotation(checkinteger(first(args)));
  #else
  (void) args;
  #endif
  return nil;
...
object *fn_invertdisplay (object *args, object *env) {
  (void) env;
  #if defined(gfxsupport)
  (*tftp).invertDisplay(first(args) != NULL);
  #else
  (void) args;
  #endif
  return nil;
...
void initgfx () {
  ...
  //tft2.init(240, 320);
  //tft2.setRotation(1);
  //tft2.fillScreen(0x820F);
  logger.config(0,1);
  logger.setTextColor(0xFFFF);
  #endif
  #endif
}
...
void setup () {
  ...
  pfstring(PSTR("uLisp 4.4a "), plog); pln(plog);
}
...
void repl (object *env) {
  for (;;) {
    ...
    if (BreakLevel) {
      ...
      pint(BreakLevel, plog);
    }
    pserial('>'); pserial(' ');
    plog('>'); plog(' ');
    Context = 0;
    object *line = read(gserial);
    //printobject(line, plog);
    //pfl(plog);
    #if defined(CPU_NRF52840)
    Serial.flush();
    #endif
    if (BreakLevel && line == nil) { pln(pserial); pln(plog); return; }
